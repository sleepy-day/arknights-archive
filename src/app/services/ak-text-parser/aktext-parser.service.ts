...
export class AKTextParserService {
  ...
  getColour(id: string): string {
    ...
    (
      Object.keys(this.richTextStyles) as (keyof typeof this.richTextStyles)[]
    ).forEach((key) => {
      if (key === id) {
  ...
  parseDescription(desc: string): string {
    ...
    while (regexPopUpNest.exec(desc) !== null) {
      ...
      desc = desc.replace(
        regexPopUpNest,
        "<div class='tooltip hl'>$3<div class='tooltiptext hl'>" +
          termDesc +
          '</div></div>'
      );
    }
  ...
  parseSkillDescription(skillInfo: object): string {
    ...
    return this.parseValuesInDescription(
      desc,
      skillInfo['blackboard' as keyof object]
    );
  }
      ...
      for (var b of blackboard) {
        ...
        if (objectKey.toLowerCase() === key.toLowerCase()) {
          if (negative) {
            if (g2 === '%') {
              value =
                String(Math.round(Math.abs(b['value' as keyof object] * 100))) +
                g2;
            } else {
      ...
      for (var b of blackboard) {
        ...
        if (objectKey.toLowerCase() === key.toLowerCase()) {
          if (negative) {
            if (g2 === '%') {
              value =
                String(Math.abs(b['value' as keyof object] * 100).toFixed(1)) +
                g2;
            } else {
            ...
            if (g2 === '%') {
              value =
                String((b['value' as keyof object] * 100).toFixed(1)) + g2;
            } else {
